...
func Test_server(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "Hello page with empty name parameter",
			URI:          "/hello?name=",
			responseCode: 400,
			body:         "",
		},
		{
			name:         "Hello page with name parameter",
		...
		},
		{
			...
			URI:          "/hello?name=Rosalind_Franklin",
			responseCode: 200,
			body:         "Hello Rosalind_Franklin!",
		},
		{
			name:         "Hello there",
			URI:          "/hello?",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req, err := http.NewRequest("GET", ts.URL+tt.URI, nil)
			if err != nil {
				t.Fatal(err)
			}
